...
for DSno in DSno_list:
	results = []
	DSno_str = str(DSno)
	df = pd.read_csv(f'DS{DSno}_timeSeries_c.csv')
	...
	for ptype in type_list:
		...
		for mode in mode_list:
			...
			plot_SNR=True, save_plots=f'{periodogram_type}_DS{DSno}_{mode}_c')
	...
	results_df.to_csv(f'bestP_results_DS{DSno}_c.csv', index=False, sep=',')
