...
def window_function(time, fmin=0.03, fmax=2.0, num_freqs=5000):
    freq = get_frequency_grid(fmin=fmin, fmax=fmax, num_freqs=num_freqs)
    ls_wf = LombScargle(time, np.ones_like(time), center_data=False, fit_mean=False)
    power_wf = ls_wf.power(freq, normalization='standard')
    return {'freq': freq, 'power': power_wf}



def GLS(time, y, dy, fmin = 0.03, fmax = 2.0, num_freqs = 5000, fit_mean=True):
    """
    ...
    ls = LombScargle(time, y, dy, center_data=False, fit_mean=fit_mean)
